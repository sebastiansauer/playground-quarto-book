...
![Der Zusammenhang von Entwicklungindex und und Korruption](img/develop-corrupt.png){#fig-develop-corrupt}
...
:::{#exm-verteilung1}
@tbl-wheels-n zeigt die Häufigkeitsverteilung von `cond` (condition, also der Zustand des Artikels, neu oder gebraucht) aus dem Datensatz `mariokart`.
...
:::{#def-balken}
### Balkendiagramm
Ein Balkendiagramm ist eine grafische Darstellung von Werten, zumeist für die Häufigkeiten bestimmter Kategorien (Ausprägungen nominaler Variablen). 
...
Eine einfache, komfortable ist die mit dem Paket `DataExplorer`, s. @fig-mario-n-plot-cond;
...
::: {.content-visible when-format="pdf"}
Zuerst importieren wir die Daten, s. @lst-mariokart-path-pdf.
:::
...
#| layout-ncol: 2
#| fig-cap: "Nicht zu wenig und nicht zu viele Balken im Balkendiagramm"
#| fig-subcap: 
...
@fig-balken-total-pr-hist-dens fügt zu @fig-balken-total-pr-hist ein *Dichtediagramm* hinzu (rote Linie). 
...
@fig-normal-interactive zeigt  interaktive Beispiele für Normalverteilung. Wählen Sie einfach Mittelwert ($\mu$) und Streuung ($\sigma$) anhand der Schieberegler.^[Quelle: <https://observablehq.com/@mcmcclur/the-normal-model>]
...
Das zeigt, dass es keinen Zusammenhang zwischen Foto und Neuwertigkeit des Spiels gibt (laut unseren Daten).
...
s. @fig-de-bar-filled. Diese Darstellung eignet sich, um Zusammenhänge zwischen zwei zweistufigen nominal skalierten Variablen zu verbildlichen. Die verschiedenen Werte der Füllfarbe werden den Stufen der Variablen `cond` zugewiesen, s. @lst-plot-bar.
...
In dem Diagramm ist eine "Trendgerade" (Regressionsgerade), um die Art des Zusammenhangs besser zu verdeutlichen.
Die Trendgerade steigt an (von links nach recht). Daraus kann man schließen: Es handelt sich um einen *gleichsinnigen* (positiven) Zusammenhang:
...
*gegensinnige* (negative) Zusammenhänge an *absteigenden* Trendgeraden $\searrow$.
...
Das Ergebnis sieht man in @fig-mario-scatter bzw. @lst-scatterplot.
...
Und wie man in @fig-zshg-nom1 sieht, ist der Anteil der Spiele mit Foto bei den neuen Spielen höher als bei gebrauchten Spielen.^[Aber Freunde lassen Freunde keine Tortendiagramme verwenden: <https://github.com/cxli233/FriendsDontLetFriends#10-friends-dont-let-friends-make-pie-chart>.]
...
Zwar sind sie beliebt, doch ist [von ihrer Verwendung zumeist abzuraten](https://www.data-to-viz.com/caveat/pie.html); vgl. auch [hier](https://github.com/cxli233/FriendsDontLetFriends#10-friends-dont-let-friends-make-pie-chart).^[<https://www.data-to-viz.com/caveat/pie.html>; <https://github.com/cxli233/FriendsDontLetFriends#10-friends-dont-let-friends-make-pie-chart>]
...
Eine weitere nützliche Art von Visualisierung sind *Karten*, *3D-Bilder* und *Animationen.* So zeigt z.B. @fig-le-world die Veränderung der Lebenserwartung (in Jahren) über die letzten Dekaden.^[Der Quellcode der Animation ist hier  zu finden: <https://gist.github.com/rafapereirabr/0d68f7ccfc3af1680c4c8353cf9ab345>.]
...
Um den gemeinsamen Zusammenhang dreier metrischer Variablen darzustellen, bietet sich ein 3D-Streudiagramm an; s. @fig-mario-3d.
...
fig2 <- plot_ly(mariokart |> filter(total_pr < 100), x = ~duration, 
                y = ~n_bids, z = ~total_pr, color = ~cond, 
...
so ist etwa [diese Animationen](https://www.tylermw.com/wp-content/uploads/2019/06/featuredmeasles.mp4) ziemlich atemberaubend.^[<https://www.tylermw.com/wp-content/uploads/2019/06/featuredmeasles.mp4>]
...
<!-- Manchmal auch wichtige Informationen. [Dieses Beispiel](https://www.autodesk.com/research/publications/same-stats-different-graphs) zeigt, -->
...
<!-- Einige Überlegungen zur Farbwahl findet sich bei @wilke_fundamentals_2019, s. [Kap. 4](https://clauswilke.com/dataviz/color-basics.html).^[Siehe auch:  <https://data-se.netlify.app/2023/06/30/farbpaletten/>] -->
...
<!-- 11. [vis-gapminder](https://datenwerk.netlify.app/posts/vis-gapminder/vis-gapminder) -->
<!-- 12. [boxplots-de1a](https://datenwerk.netlify.app/posts/boxplots-de1a/boxplots-de1a) -->
<!-- 13. [diamonds-histogramm-vergleich](https://datenwerk.netlify.app/posts/diamonds-histogramm-vergleich/diamonds-histogramm-vergleich) -->
<!-- 14. [wozu-balkendiagramm](https://datenwerk.netlify.app/posts/wozu-balkendiagramm/wozu-balkendiagramm) -->
<!-- 15. [diamonds-histogram](https://datenwerk.netlify.app/posts/diamonds-histogram/diamonds-histogram) -->
<!-- 16. [n-vars-diagram](https://datenwerk.netlify.app/posts/n-vars-diagram/n-vars-diagram) -->

<!-- Noch mehr Aufgaben zum Thema Datenvisualisierung finden Sie im Datenwerk unter dem Tag [vis](https://datenwerk.netlify.app/#category=vis). -->
...
<!-- ::: {.content-visible when-format="html"} -->
<!-- [Hier](https://www.bing.com/search?pc=OA1&q=introductory%20books%20on%20data%20visualization%20with%20DOI) ist eine Liste von Büchern zum Thema; dort können Sie bei Interesse tiefer suchen. -->
<!-- ::: -->
